...
wss.on("connection", (ws) => {
  ws.on("message", (message) => {
    ...
    if (data.type === "join") {
      const player = { ws, isAdmin: false, username: data.username };  // Store username for reference
      players.push(player);
    ...
    if (data.type === "start" && ws === admin.ws) {
      ...
      // Randomly select imposters using a shuffle method
      let shuffledPlayers = [...players];  // Create a copy of the players array
      shuffledPlayers.sort(() => Math.random() - 0.5);  // Shuffle the array randomly

      const imposters = new Set(shuffledPlayers.slice(0, imposterCount));  // Pick the first N shuffled players as imposters

      // Send word to all players and let imposters know each other
      players.forEach(player => {
        if (imposters.has(player)) {
          const imposterNames = Array.from(imposters).map(p => p.username); // Get names of imposters
          player.ws.send(JSON.stringify({
            type: "gameStart",
            word: null,  // Imposter gets no word
            imposters: imposterNames  // Imposter gets the list of all imposter names
          }));
        } else {
          player.ws.send(JSON.stringify({
            type: "gameStart",
            word: chosenWord  // Non-imposters get the game word
          }));
        }
      });
    }
